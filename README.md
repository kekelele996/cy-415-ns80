...
- 交换改选：物主可以从申请人当前可交换的物品里另选一件，提交后待申请人确认；接受后交换对象以改选结果为准，拒绝则请求结束、双方物品都不变。
...
- 物主不必先拒绝，可以在待确认请求上直接「改选」：从申请人当前可交换（available）的物品中另选一件提交，请求进入 `reselecting`（改选待确认）状态。
- 改选期间不锁定、不改动任何物品，原候选物品仍可被别人申请。
- 申请人接受改选后，`from_item_id` 覆盖为改选结果，请求变为「已同意」；拒绝则请求变为「已拒绝」并结束，双方物品都不变。
- 改选不能写入的情形：请求已处理（含改选待确认/已同意/已拒绝/已完成）、所选物品不属于申请人或不再可交换、物主物品归属已变化。
- 交换完成时只把最终约定的两件物品标记为已交换，原候选物品若被改选掉则保持原状。

### ExchangeStatus 取值

`pending`（待确认）、`reselecting`（改选待确认）、`accepted`（已同意）、`rejected`（已拒绝）、`completed`（已完成）。
